    ...
    this.state={
      editingEntries: {
        'vehicle': {
          ...
          "insurance_start": moment(this.props.navigation.insurStartDate).format("YYYY-MM-DD"),
          "insurance_stop": moment(this.props.navigation.insurEndDate).format("YYYY-MM-DD"),
        }
      },
      picker1: false,
      picker2: false,
        ...
        <ScrollView>
          <Block middle>
            <KeyboardAwareScrollView>
              ...
              <TextInput
                onChangeText = {(text)=> this.setState({editingEntries:{vehicle:{car_make: text}}})}
                placeholder = {editingEntries.vehicle.car_make}
                returnKeyType={'next'}
                onSubmitEditing={()=>{this.carModel.focus();}}
                blurOnSubmit = {false}
                style = {[styles.saeInputAlt]}
              ...
              <TextInput
                onChangeText={(text) => this.setState({editingEntries:{vehicle:{car_model: text}}})}
                placeholder ={editingEntries.vehicle.car_model}
                ref={(input) => {this.carModel = input;}}
                returnKeyType = {'next'}
                onSubmitEditing = { () => {this.carYear.focus();}}
                blurOnSubmit = {false}
              ...
              <Block style={styles.footer}>
                <CalendarButton 
                  title="Continue" 
                  onPress={() => 
                    this.handleEditSubmit(editingEntries, this.props.navigation)} 
                />
              </Block>
            </KeyboardAwareScrollView>
          </Block>
...
render(){
  ...
  <View>
    ...
    {isEditing ? 
      (this.renderEditingVehicle()) 
        :
      (this.renderAddingVehicle())
    }
  </View>
  )
}
