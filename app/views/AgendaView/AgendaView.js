...
class AgendaView extends React.Component {
    ...
    if (item.isRecurring === true) {
      ...
      for (const each of response) {
        ...
        if (each.isRecurring === true) {
          ...
          console.log(
            'the last date of all matching items is: ',
            lastDate[lastDate.length - 1]
          );
          return updatedRecur;
        }
      }
  ...
  renderItem = item => {
    ...
    console.log('the result of testAMatch function is: ', endDate);
    let ending = moment(endDate).format('MMMM D, YYYY');
    console.log('reformatted end date of recurrences: ', ending);
